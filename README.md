...
Airflow DAG оркестрирует тот же production pipeline поверх CLI-команд проекта:
...
Запуск Airflow в локальном окружении через `uv`:
...
Логин и пароль для локального запуска:

```text
admin / admin
```

`airflow standalone` может напечатать сгенерированный пароль, если локальный
пользователь уже существовал. В таком случае используйте пароль из вывода
команды.

Запустите DAG `landscape_training_pipeline` вручную из UI или из второго
терминала:
...
По умолчанию DAG обращается к MLflow по адресу `http://127.0.0.1:8080`.
При необходимости путь к проекту и MLflow URI можно переопределить переменными:

```bash
LANDSCAPE_PROJECT_DIR="$PWD"
